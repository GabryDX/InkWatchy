...
void writeTextReplaceBack(const String &str, int16_t x, int16_t y, uint16_t frColor = SCBlack, uint16_t bgColor = SCWhite, bool manualWidth = false, uint8_t manualWidthAdd = 1, uint8_t manualHeighAdd = 3);
void centerText(const String &str, uint16_t *currentHeight);
void writeTextCenterReplaceBack(const String &str, uint16_t y, uint16_t frColor = SCBlack, uint16_t bgColor = SCWhite);
void getTextBounds(const String &str, int16_t *xa, int16_t *ya, uint16_t *wa, uint16_t *ha, int16_t cxa = 1, int16_t cya = 100);
